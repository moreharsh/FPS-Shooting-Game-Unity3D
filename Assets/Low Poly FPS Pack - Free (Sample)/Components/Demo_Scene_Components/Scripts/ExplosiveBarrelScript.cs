...
public class ExplosiveBarrelScript : MonoBehaviour {
	...
	private IEnumerator Explode () {
        ...
        Collider[] collidersToDestroy = Physics.OverlapSphere(transform.position, 35f);
        
        foreach (Collider nearbyObject in collidersToDestroy)
        {
            PlayerHealth dest = nearbyObject.GetComponent<PlayerHealth>();
            if(dest != null)
            {
                dest.lowerHealthByBarrel(50f);
            }

			PlayerHealthGun dest1 = nearbyObject.GetComponent<PlayerHealthGun>();
            if(dest1 != null)
            {
                dest1.lowerHealthByBarrel(50f);
            }
			
			EnemyHealth dest2 = nearbyObject.GetComponent<EnemyHealth>();
            if(dest2 != null)
            {
                dest2.lowerHealth(50f);
            }
